			...
			if (in_array ( 'OPT_FB_CACHE', $camp_opt )) {
				...
				if (stristr ( $temp, 'messages' )) {
					...
					if ($cg_fb_source == 'group') {
						...
						preg_match ( '!name="fb_dtsg" value="(.*?)"!', $exec, $found_dtsg );
						$found_dtsg = ($found_dtsg [1]);
						echo '<br>dtsg tag:' . $found_dtsg;
						
						// curl post
						$curlpost = "m_sess=&fb_dtsg=" . urlencode ( $found_dtsg ) . "&__csr=&__req=d&__a=";
...
*/				
				...
				foreach ( $items as $item ) {
					...
					} elseif (stristr ( $exec2, '"GenericAttachmentMedia"' )) {
					...
					if ($isEvent == true) {
						...
						preg_match ( '!"utc_start_timestamp":(.*?),"utc_end_timestamp":(.*?)}!', $exec2, $date_matches );
						...
						if (isset ( $date_matches [1] ) && trim ( $date_matches [1] ) != '') {
							...
							//end date if exists
							if (isset ( $date_matches [2] ) && is_numeric($date_matches [2]) ) {
						...
						if (isset ( $address_matches [1] ) && trim ( $address_matches [1] ) != '') {
							$place_address = $address_matches [1] . '<br>' . $address_matches [2] ;
						}else{
						...
						$ret ['place_street'] = (isset($street_matches[1]) && trim($street_matches[1]) != '' ) ?  $street_matches[1] : '';

						//"city":{"contextual_name":"Tanta"
						preg_match ( '!"city":{"contextual_name":"(.*?)"!', $exec2, $city_matches );
						$ret ['place_city'] = (isset($city_matches[1]) && trim($city_matches[1]) != '' ) ?  $city_matches[1] : '';
						...
						$name_arr = json_decode('["' . $from_matches[1] . '"]') ;
						 
						$from_name = $name_arr [0];
...
(function(d, s, id) {
    ...
    fjs.parentNode.insertBefore(js, fjs);
				...
				if (in_array ( 'OPT_FB_CACHE', $camp_opt )) {
					...
					if ( stristr (   $list_exec_raw , '"see_more_cards_id\",\"href\"' ) || stristr ( $list_exec_raw, 'm_more_item\",\"href' ) || stristr ( $list_exec_raw, 'profile\\\\\\/timeline\\\\\\/stream' ) || (stristr ( $exec, 'serialized_cursor' ) && $cg_fb_from == 'events')) {
