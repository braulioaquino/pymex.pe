...
function ebay_fetch_items($keyword, $camp) {
	...
	$wp_automatic_ebay_app = get_option('wp_automatic_ebay_app' , '');
	...
	$elink .= '&GLOBAL-ID=' . wp_automatic_fix_category($camp_general['cg_eb_site']) ;
	...
	$i = 0;
	  ...
	  foreach ( $search_results as $item_new ) {
		...
		$itm ['item_subtitle'] = $item_new->subtitle[0];
		$itm ['item_category'] = $item_new->primaryCategory[0]->categoryName[0];
		$itm ['item_payment'] = $item_new->paymentMethod[0];
		$itm ['item_postal'] = $item_new->postalCode[0];
		...
		$itm ['item_bids'] =  $item_new->sellingStatus[0]->bidCount[0] ;
		$itm ['item_price'] = $item_new->sellingStatus[0]->currentPrice[0]->__value__ ;
		$itm ['item_bin'] =    $item_new->listingInfo[0]->buyItNowPrice[0]->__value__;
